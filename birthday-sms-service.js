...
async function sendBirthdaySMS(user) {
    ...
    const testPhone = process.env.TEST_PHONE_NUMBER || '+2347065649583'; // fallback test number
  
    const toPhone = isDevelopment ? testPhone : user.phone.replace(/\s+/g, '');
  
    const messageBody = `Happy Birthday ${user.name}! 🎂 Wishing you a fantastic day filled with joy and celebration. Best wishes from TLBC.`;
    ...
    try {
      ...
      console.log(`SMS sent to ${user.name} (${toPhone}) [Dev: ${isDevelopment}], SID: ${message.sid}`);
      return true;
    } catch (error) {
      console.error(`Error sending SMS to ${user.name} (${toPhone}):`, error);
      return false;
    }
  }
...
async function sendTestSMS() {
  try {
    const testPhone = '+2348064430141'; // Your test phone number
    
    const message = await twilioClient.messages.create({
      body: `Happy Birthday dear esteemed Pastor Eloka. We all are grateful to God for the gift you are to us. We love you, Sir`,
      from: process.env.TWILIO_PHONE_NUMBER,
      to: testPhone
    });
  ...
  (async () => {
    try {
      ...
      if (birthdayUsers.length > 0) {
        ...
        await sendBirthdaySMS(birthdayUsers[0]);
      } else {
